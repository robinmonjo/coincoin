defmodule Coincoin.Blockchain.Test.MempoolTest do
  use ExUnit.Case, async: false

  alias Coincoin.Blockchain.{Mempool, Block}

  test "handle_call mine_data add data to pool and start mining" do
    data = "foo"
    {:reply, response, new_state} = Mempool.handle_call({:mine, data}, nil, {[], {}})
    assert response == :ok
  ...
  test "handle_call mine_data do not change state if data already in pool" do
    data = "foo"
    state = {[data], {}}
    {:reply, response, new_state} = Mempool.handle_call({:mine, data}, nil, state)
  ...
  test "handle_call mine_data only add to pool if mining in progress" do
    ...
    {:reply, response, new_state} = Mempool.handle_call({:mine, data}, nil, {pool, mining})
    assert response == :ok
  ...
  test "handle_call block_mined remove mined block data from the pool" do
    ...
    {:reply, response, new_state} = Mempool.handle_call({:block_mined, b}, nil, {pool, {}})
    assert response == :ok
    {pool, {}} = new_state
    assert pool == ["bar"]
  end
