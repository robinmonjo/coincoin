defmodule Coincoin.Token.Wallet do
  @moduledoc """
  Structure that represents a wallet and functions to work on wallet
  """

  alias Coincoin.Token.{Wallet, Crypto, Ledger, Transaction}
  ...
  def sign(message, %Wallet{private_key: key}) do
    Crypto.sign(key, message)
  end

  @spec verify(String.t(), String.t(), t) :: boolean
  def verify(message, signature, %Wallet{public_key: key}) do
    Crypto.verify_signature(key, message, signature)
  end
  ...
  def send(value, recipient, %Wallet{} = wallet) do
    # @TODO remove with statement all of the match no matter what
